...
public class SetWalkSpeed implements CommandExecutor {
    private final JavaPlugin plugin;
    public SetWalkSpeed(JavaPlugin plugin) {
        this.plugin = plugin;
    }
    @Override
        ...
        if((sender instanceof Player && sender.isOp()) || sender instanceof ConsoleCommandSender) {
            try {
                if (args.length == 2 && Bukkit.getPlayer(args[0]) == null) {
                    ...
                    ((Player) sender).setWalkSpeed(Float.parseFloat(args[0]) / 10);
                    sender.sendMessage(ChatColor.GREEN + "Вы успешно установили себе скорость ходьбы: " + args[0] + ChatColor.RESET);
                } else if (sender instanceof Player || sender instanceof ConsoleCommandSender && args.length == 2) {
                    plugin.getServer().getPlayer(args[0]).setWalkSpeed(Float.parseFloat(args[1]) / 10);
                    sender.sendMessage(ChatColor.GREEN + "Вы успешно установили для " + args[0] + " скорость ходьбы: " + args[1] + ChatColor.RESET);
                }
            } catch (Exception e) {
                try {
                    ...
                    sender.sendMessage(ChatColor.RED + "/setflyspeed <ник игрока> <скорость ходьбы> - если вы хотите установить скорость конкретному игроку ");
                    sender.sendMessage(ChatColor.RED + "/setflyspeed <скорость ходьбы> - если вы хотите установить скорость себе (консоль не может ставить скорость себе) ");
                }
            }
